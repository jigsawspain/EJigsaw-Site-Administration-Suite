...
class EJ_adverts
{
	...
	function EJ_adverts ($EJ_mysql, $_vars, $_settings)
	{
	...
	function install()
	{
		...
		if ($this->EJ_mysql->numRows() == 2)
		{
			echo "
			<p class=\"EJ_instText\">
				&gt;&gt; EJ_adverts tables already found!<br/>
				&gt;&gt; Checking default settings
			</p>";
		} else
		{
			// Main Adverts Table
			$this->EJ_mysql->query("CREATE TABLE IF NOT EXISTS {$this->EJ_mysql->prefix}module_EJ_adverts (
			...
			$this->EJ_mysql->query("CREATE TABLE IF NOT EXISTS {$this->EJ_mysql->prefix}module_EJ_adverts_settings (
				setting VARCHAR(20) NOT NULL ,
				value VARCHAR(100) NOT NULL ,
				PRIMARY KEY (setting)
				)");
			$this->EJ_mysql->query("SHOW TABLES LIKE '{$this->EJ_mysql->prefix}module_EJ_adverts_settings'");
			if ($this->EJ_mysql->numRows()!=1) return false;
			// Advert Categories Table
			$this->EJ_mysql->query("CREATE TABLE IF NOT EXISTS {$this->EJ_mysql->prefix}module_EJ_adverts_cats (
			...
			$this->EJ_mysql->query("SHOW TABLES LIKE '{$this->EJ_mysql->prefix}module_EJ_adverts_cats'");
			if ($this->EJ_mysql->numRows()!=1) return false;
			// Advert Attributes Table
			$this->EJ_mysql->query("CREATE TABLE IF NOT EXISTS {$this->EJ_mysql->prefix}module_EJ_adverts_atts (
				attId INT(6) NOT NULL AUTO_INCREMENT,
				attName VARCHAR(30) NOT NULL ,
				attDesc TEXT ,
				PRIMARY KEY (attId)
				)");
			$this->EJ_mysql->query("SHOW TABLES LIKE '{$this->EJ_mysql->prefix}module_EJ_adverts_atts'");
			if ($this->EJ_mysql->numRows()!=1) return false;
			// Advert Locations Table
			$this->EJ_mysql->query("CREATE TABLE IF NOT EXISTS {$this->EJ_mysql->prefix}module_EJ_adverts_locs (
				locId INT(10) NOT NULL AUTO_INCREMENT,
				locName VARCHAR(50) NOT NULL ,
				locLevel TINYINT(1) NOT NULL DEFAULT 0 ,
				PRIMARY KEY (locId)
				)");
			$this->EJ_mysql->query("CREATE TABLE IF NOT EXISTS {$this->EJ_mysql->prefix}module_EJ_adverts_hits (
				adId INT(11) NOT NULL,
				hitMonth VARCHAR(4) NOT NULL ,
				hits INT(7) NOT NULL DEFAULT 0
				)");
			$this->EJ_mysql->query("SHOW TABLES LIKE '{$this->EJ_mysql->prefix}module_EJ_adverts_locs'");
			if ($this->EJ_mysql->numRows()!=1) return false;
			// Create initial advert
			$this->EJ_mysql->query("SELECT catId FROM {$this->EJ_mysql->prefix}module_EJ_adverts_cats");
			...
			if ($this->EJ_mysql->numRows()==0)
			{
				...
				$this->EJ_mysql->query("INSERT INTO {$this->EJ_mysql->prefix}module_EJ_adverts_cats SET subCatOf = NULL, catname = 'Default Category', catDesc = 'This is the default category set up by EJ_adverts'");
			}
			// Create initial attributes
			$this->EJ_mysql->query("SELECT attId FROM {$this->EJ_mysql->prefix}module_EJ_adverts_atts");
			if ($this->EJ_mysql->numRows()==0)
			{
				echo "
				<p class=\"EJ_instText\">
				...
				$this->EJ_mysql->query("INSERT INTO {$this->EJ_mysql->prefix}module_EJ_adverts_atts SET attName = 'Default Attribute', attDesc = 'This is the default attribute set up by EJ_adverts'");
			}
			// Create initial locations
			$this->EJ_mysql->query("SELECT locId FROM {$this->EJ_mysql->prefix}module_EJ_adverts_locs");
			if ($this->EJ_mysql->numRows()==0)
			{
				echo "
				<p class=\"EJ_instText\">
				...
				$this->EJ_mysql->query("INSERT INTO {$this->EJ_mysql->prefix}module_EJ_adverts_locs SET locName = 'Default Location'");
			}
		}
		...
		$this->EJ_mysql->query("SHOW COLUMNS FROM {$this->EJ_mysql->prefix}users LIKE 'perm_EJ_adverts'");
		if ($this->EJ_mysql->numRows()==0)
		{
			$this->EJ_mysql->query("ALTER TABLE {$this->EJ_mysql->prefix}users ADD perm_EJ_adverts TINYINT(1) NOT NULL DEFAULT 0");
		}
		$this->EJ_mysql->query("UPDATE {$this->EJ_mysql->prefix}users SET perm_EJ_adverts = 1 WHERE userid = 'admin'");
		// Check / create initial settings
		echo "
			<p class=\"EJ_instText\">
			&gt; Creating initital settings...
			</p>";
		$this->EJ_mysql->query("INSERT INTO {$this->EJ_mysql->prefix}module_EJ_adverts_settings (setting, value) VALUES
			('small_width', '250px') ,
			('small_height', '350px') ,
			('small_articles', '3') ,
			('small_word_count', '25') ,
	...
	function uninstall()
	{
		...
		$this->EJ_mysql->query("DROP TABLE IF EXISTS {$this->EJ_mysql->prefix}module_EJ_adverts");
		$this->EJ_mysql->query("DROP TABLE IF EXISTS {$this->EJ_mysql->prefix}module_EJ_adverts_settings");
		$this->EJ_mysql->query("DROP TABLE IF EXISTS {$this->EJ_mysql->prefix}module_EJ_adverts_cats");
		$this->EJ_mysql->query("DROP TABLE IF EXISTS {$this->EJ_mysql->prefix}module_EJ_adverts_atts");
		$this->EJ_mysql->query("DROP TABLE IF EXISTS {$this->EJ_mysql->prefix}module_EJ_adverts_locs");
		echo "
			<p class=\"EJ_instText\">
		...
		if ($this->EJ_mysql->numRows()!=0)
		{
			$this->EJ_mysql->query("ALTER TABLE {$this->EJ_mysql->prefix}users DROP perm_EJ_adverts");
		}
		echo "
			<p class=\"EJ_instText\">
			&gt; Updating Module Registry...
	...
	{
		$content = "";
		$content .= '<a class="button" style="background-image: url('.$this->moduleloc.'add_icon.png)" href="./?module=EJ_adverts&action=addadvert">Add Advert</a><a class="button" style="background-image: url('.$this->moduleloc.'search_icon.png)" href="./?module=EJ_adverts&action=search">Advert Search</a><a class="button" style="background-image: url('.$this->moduleloc.'cats_icon.png)" href="./?module=EJ_adverts&action=cats">Categories</a><a class="button" style="background-image: url('.$this->moduleloc.'atts_icon.png)" href="./?module=EJ_adverts&action=atts">Attributes</a><a class="button" style="background-image: url('.$this->moduleloc.'locs_icon.png)" href="./?module=EJ_adverts&action=locs">Locations</a>';
		echo $content;
	}
	
	function search()
	{
		$content = '<div style="text-align: center; margin-top: 3px;"><a href="?module=EJ_adverts&action=admin_page"><img src="'.$this->moduleloc.'back.png" alt="Back to adverts" title="Back to adverts" style="border:0;" /></a></div>';
		$content .= '<h2><img src="'.$this->moduleloc.'search_icon_small.png" alt="Advert Filter" /> Advert Filter</h2>';
		$results = array();
		$query = "SELECT SQL_CALC_FOUND_ROWS * FROM {$this->EJ_mysql->prefix}module_EJ_adverts WHERE 1=1";
		$advertdate = time();
		$adverttext = "";
		$advertcat = "";
		$advertposter = "";
		...
		$content .= '
			<script src="'.$this->moduleloc.'EJ_adverts.js" language="javascript" type="text/javascript"></script>
			<script src="'.$this->moduleloc.'calendar.js" language="javascript" type="text/javascript"></script>
			<form name="search_form" id="search_form" method="post" action="./?module=EJ_adverts&action=search&search=go">
		...
		while ($advert = $this->EJ_mysql->getRow())
		{
			...
			if (empty($advert['EJ_advertImages']) or !file_exists($this->moduleloc."images/".$advert['EJ_advertImages']))
			{
				$img = "noimage.png";
			} else
			{
				$img = $advert['EJ_advertImages'];
			}
			$content3 .= "
...
';
				...
				<div id="addadvert">
					<form name="add_form" id="add_form" action="?module=EJ_adverts&action=addadvert" method="post">
						<div id="addLeft">
							...
							<img id="advertimage" src="'.$this->moduleloc.'images/noimage.png" alt="Add An Image" title="Click to Add an Image" onclick="changepic()" style="width:200px; height:200px;" /><br/>
	...
	function editadvert()
	{
		...
		$this->EJ_mysql->query("SELECT * FROM {$this->EJ_mysql->prefix}module_EJ_adverts WHERE EJ_advertId = ".$this->vars['advertid']);
		...
		} else
		{
			...
			$content .= '
				...
				<script src="'.$this->moduleloc.'addcalendar.js" language="javascript" type="text/javascript"></script>
				<div id="addadvert">
					<form name="add_form" id="add_form" action="?module=EJ_adverts&action=editadvert" method="post">
						<div id="addLeft">
							Click Image To Change<br/>
							<img id="advertimage" src="'.$this->moduleloc.'images/'.$img.'" alt="Change Image" title="Click to Change Image" onclick="changepic()"  style="width:200px; height:200px;" /><br/>
			...
			$this->EJ_mysql->query("SELECT hits FROM {$this->EJ_mysql->prefix}module_EJ_adverts_hits WHERE adId = '{$this->vars['advertid']}' and hitMonth = '".date("my")."'");
			if ($this->EJ_mysql->numRows() == 0)
			{
				$hits[0] = 0;
			}
			...
			$this->EJ_mysql->query("SELECT hits FROM {$this->EJ_mysql->prefix}module_EJ_adverts_hits WHERE adId = '{$this->vars['advertid']}' and hitMonth = '".date("my", strtotime('-1 Month'))."'");
			if ($this->EJ_mysql->numRows() == 0)
			{
				$hits[1] = 0;
			}
	...
	function cats()
	{
		$content = '<div style="text-align: center; margin-top: 3px;"><a href="?module=EJ_adverts&action=admin_page"><img src="'.$this->moduleloc.'back.png" alt="Back to adverts" title="Back to adverts" style="border:0;" /></a></div>
				...
				<script src="'.$this->moduleloc.'EJ_adverts.js" language="javascript" type="text/javascript"></script>';
		$this->EJ_mysql->query("SELECT *,(SELECT COUNT(*) FROM {$this->EJ_mysql->prefix}module_EJ_adverts WHERE EJ_advertCat = catId) AS adverts FROM {$this->EJ_mysql->prefix}module_EJ_adverts_cats ORDER BY subCatOf ASC, catName ASC");
		while ($cat = $this->EJ_mysql->getRow())
		{
			$cats[$cat['catId']] = $cat;
		}
		foreach ($cats as $cat)
		{
			...
			if (empty($cat['subCatOf']))
			{
				...
				$content .= "<a href=\"?module=EJ_adverts&action=editcat&catid={$cat['catId']}\"><img src=\"".$this->moduleloc."edit.png\" alt=\"edit\" title=\"Edit Category\" style=\"cursor: pointer; border: 0;\" /></a>";
				if (count($subcats)!=0)
				{
				...
				$content .= ")";
				if ($cat['adverts']!=0) $content .= " <a href=\"?module=EJ_adverts&action=search&search=EJ_advertCat::{$cat['catId']}\"><img src=\"".$this->moduleloc."search_icon_small.png\" alt=\"Find\" title=\"Show adverts in this category\" /></a>";
				$i = 1;
				if (count($subcats) != 0)
				{
					foreach($subcats as $subcat)
					{
						...
						if ($subcat['adverts']!=0) $content .= " <a href=\"?module=EJ_adverts&action=search&search=EJ_advertCat::{$subcat['catId']}\"><img src=\"".$this->moduleloc."search_icon_small.png\" alt=\"Find\" title=\"Show adverts in this category\" /></a>";
						$content .= " <a href=\"?module=EJ_adverts&action=editcat&catid={$subcat['catId']}\"><img src=\"".$this->moduleloc."edit.png\" alt=\"edit\" title=\"Edit Category\" style=\"cursor: pointer; border: 0;\" /></a>";
						if ($subcat['adverts']==0)
						{
	...
	function editcat()
	{
		...
		$this->EJ_mysql->query("SELECT * FROM ".$this->EJ_mysql->prefix."module_EJ_adverts_cats WHERE catId = ".$this->vars['catid']);
		if ($this->EJ_mysql->numRows()!=1)
		{
			$content .= '
					...
					<form name=\"new_cat_form\" id=\"new_cat_form\" method=\"post\" action=\"#\" style=\"margin: 10px;\">
						...
						<div style=\"float:left; margin-right: 5px;\">
							...
							<select name=\"new_sub\" id=\"new_sub\">
								...
								";
					$this->EJ_mysql->query("SELECT * FROM ".$this->EJ_mysql->prefix."module_EJ_adverts_cats WHERE (ISNULL(subCatOf) OR subCatOf = '') AND catId != ".$cat['catId']);
	...
	function atts()
	{
		$content = '<div style="text-align: center; margin-top: 3px;"><a href="?module=EJ_adverts&action=admin_page"><img src="'.$this->moduleloc.'back.png" alt="Back to Adverts" title="Back to Adverts" style="border:0;" /></a></div>
				...
				<script src="'.$this->moduleloc.'EJ_adverts.js" language="javascript" type="text/javascript"></script>';
		$this->EJ_mysql->query("SELECT *,(SELECT COUNT(*) FROM {$this->EJ_mysql->prefix}module_EJ_adverts WHERE EJ_advertAttributes LIKE CONCAT('%(', attId, ')%')) AS adverts FROM {$this->EJ_mysql->prefix}module_EJ_adverts_atts ORDER BY attName ASC");
		while ($cat = $this->EJ_mysql->getRow())
		{
			$cats[$cat['attId']] = $cat;
		}
		foreach ($cats as $cat)
		{
			...
			$content .= "<a href=\"?module=EJ_adverts&action=editatt&attid={$cat['attId']}\"><img src=\"".$this->moduleloc."edit.png\" alt=\"edit\" title=\"Edit Attribute\" style=\"cursor: pointer; border: 0;\" /></a>";
			$content .= "</div>";
			$content .= "<img class=\"advert_cat_img\"src=\"{$this->moduleloc}att.png\" alt=\"\" />";
			$content .= " {$cat['attName']} ({$cat['adverts']}";
			$content .= ")";
			if ($cat['adverts']!=0) $content .= " <a href=\"?module=EJ_adverts&action=search&search=EJ_advertAttributes::{$cat['attId']}\"><img src=\"".$this->moduleloc."search_icon_small.png\" alt=\"Find\" title=\"Show adverts with this attribute\" /></a>";
			$i = 1;
			$content .= "</div>";
		}
		$content .= "
	...
	function editatt()
	{
		...
		$this->EJ_mysql->query("SELECT * FROM ".$this->EJ_mysql->prefix."module_EJ_adverts_atts WHERE attId = ".$this->vars['attid']);
		if ($this->EJ_mysql->numRows()!=1)
		{
			$content .= '
	...
	{
		$content = '<div style="text-align: center; margin-top: 3px;"><a href="?module=EJ_adverts&action=admin_page"><img src="'.$this->moduleloc.'back.png" alt="Back to Adverts" title="Back to Adverts" style="border:0;" /></a></div>
				<h2><img src="'.$this->moduleloc.'atts_icon_small.png" alt="Locations" /> Locations</h2>
				<script src="'.$this->moduleloc.'EJ_adverts.js" language="javascript" type="text/javascript"></script>';
		$this->EJ_mysql->query("SELECT *,(SELECT COUNT(*) FROM {$this->EJ_mysql->prefix}module_EJ_adverts WHERE EJ_advertLoc LIKE CONCAT('%(', locId, ')%')) AS adverts FROM {$this->EJ_mysql->prefix}module_EJ_adverts_locs ORDER BY locName ASC");
		while ($cat = $this->EJ_mysql->getRow())
		{
			$cats[$cat['locId']] = $cat;
		}
		foreach ($cats as $cat)
		{
			...
			$content .= "<a href=\"?module=EJ_adverts&action=editloc&locid={$cat['locId']}\"><img src=\"".$this->moduleloc."edit.png\" alt=\"edit\" title=\"Edit Location\" style=\"cursor: pointer; border: 0;\" /></a>";
			$content .= "</div>";
			$content .= "<img class=\"advert_cat_img\"src=\"{$this->moduleloc}loc.png\" alt=\"\" />";
			$content .= " {$cat['locName']} ({$cat['adverts']}";
			$content .= ")";
			if ($cat['adverts']!=0) $content .= " <a href=\"?module=EJ_adverts&action=search&search=EJ_advertLoc::{$cat['locId']}\"><img src=\"".$this->moduleloc."search_icon_small.png\" alt=\"Find\" title=\"Show adverts in this location\" /></a>";
			$i = 1;
			$content .= "</div>";
		}
		$content .= "
	...
	function editloc()
	{
		...
		$this->EJ_mysql->query("SELECT * FROM ".$this->EJ_mysql->prefix."module_EJ_adverts_locs WHERE locId = ".$this->vars['locid']);
		if ($this->EJ_mysql->numRows()!=1)
		{
			$content .= '
	...
	function advert_categories()
	{
		...
		} else
		{
			...
			foreach ($cats as $cat)
			{
				...
				$content .= '
					<li style="float:left; margin-bottom: 10px; width: 23%;"><a href="?module='.get_class($this).'&action=show_ads&cat='.$cat['thiscat']['catId'].'">'.$cat['thiscat']['catName'].'</a> ('.$cat['thiscat']['catCount'].')';
				if (!empty($cat['subcats']) and count($cat['subcats']!=0))
				{
					...
					{
						$content .= '
							<li><a href="?module='.get_class($this).'&action=show_ads&cat='.$subcat['catId'].'">'.$subcat['catName'].'</a> ('.$subcat['catCount'].')</li>';
					}
					$content .= '
						</ul>';
				}
	...
	function show_ads()
	{
		...
		$filter .= "
				...
				<p>
					<strong>Category:</strong><select name=\"category\" id=\"category\" onchange=\"updateAdvertFilter('{$_SESSION['key']}','{$this->EJ_settings['instloc']}');\">
						<option value=\"0\">Any Category</option>";
		$this->EJ_mysql->query("SELECT * FROM {$this->EJ_mysql->prefix}module_EJ_adverts_cats");
		while ($cat = $this->EJ_mysql->getRow())
		{
			if ($_REQUEST['cat']==$cat['catId'])
				$selected = " selected=\"selected\"";
			else
				$selected = "";
			$filter .= "
		...
		{
			$query .= " and EJ_advertCat = '{$this->vars['cat']}'";
		}
		if (isset($this->vars['loc']))
		{
		...
		} else
		{
			while($advert = $this->EJ_mysql->getRow())
			{
				...
				if (!empty($advert['EJ_advertImages']) and file_exists(dirname(__FILE__)."/EJ_adverts/images/".$advert['EJ_advertImages']))
				{
					$image = "<img class=\"EJ_advertResult_img\" src=\"{$this->EJ_settings['instloc']}{$this->moduleloc}image.php/{$advert['EJ_advertImages']}?image={$this->EJ_settings['instloc']}{$this->moduleloc}images/{$advert['EJ_advertImages']}&amp;height=100&amp;width=100\" alt=\"{$advert['EJ_advertTitle']}\"/>";
				} else
				{
	...
	function show_advert()
	{
		...
		{
			$query = "SELECT * FROM {$this->EJ_mysql->prefix}module_EJ_adverts WHERE EJ_advertId = {$_REQUEST['adId']}";
			$this->EJ_mysql->query($query);
			if ($this->EJ_mysql->numRows() == 0)
			{
				$content .= '<p style="text-align: center;"><strong>Listing Not Found!<br/>Please go back and try again.</strong></p>';
			} else
			{
				...
				else
				{
					...
					if ($this->EJ_mysql->numRows() == 0)
					{
						$this->EJ_mysql->query("INSERT INTO {$this->EJ_mysql->prefix}module_EJ_adverts_hits SET adId = '{$advert['EJ_advertId']}', hitMonth = '".date("my")."', hits = 1");
					}
					else
					{
					...
					if (!empty($advert['EJ_advertWebsite'])) $content .= "<br/><img src=\"{$this->EJ_settings['instloc']}{$this->moduleloc}globe.png\" alt=\"Website\"/> <a href=\"{$advert['EJ_advertWebsite']}\" target=\"_blank\">{$advert['EJ_advertWebsite']}</a>";
					$content .= "</div>";
					...
					$content .= $form."</div>";
					$content .= "<div id=\"EJ_advertResult_left\"><div id=\"EJ_advertResult_mainLeft\">{$advert['EJ_advertText']}";
					$content .= "</div><div id=\"EJ_advertResult_mainRight\">";
					if (!empty($advert['EJ_advertImages']) and file_exists(dirname(__FILE__)."/EJ_adverts/images/".$advert['EJ_advertImages']))
					{
						$image = "<img src=\"{$this->EJ_settings['instloc']}{$this->moduleloc}image.php/{$advert['EJ_advertImages']}?image={$this->EJ_settings['instloc']}{$this->moduleloc}images/{$advert['EJ_advertImages']}&amp;height=298&amp;width=298\" alt=\"{$advert['EJ_advertTitle']}\"/>";
					} else
					{
	...
	{
		$this->EJ_mysql->query("SELECT adId, hits FROM {$this->EJ_mysql->prefix}module_EJ_adverts_hits WHERE (SELECT EJ_advertHidden FROM {$this->EJ_mysql->prefix}module_EJ_adverts WHERE EJ_advertId = adId) != 1 GROUP BY adId ORDER BY SUM(hits) DESC LIMIT 5");
		$count = 0;
		...
		foreach ($hits as $hit)
		{
			$this->EJ_mysql->query("SELECT * FROM {$this->EJ_mysql->prefix}module_EJ_adverts WHERE EJ_advertId = '{$hit['adId']}'");
			$advert = $this->EJ_mysql->getRow();
			if (strrpos($advert['EJ_advertLoc'],"(")!=0)
			{
				$advert['locName'] = "Multiple Locations";
			...
			if (!empty($advert['EJ_advertImages']) and file_exists(dirname(__FILE__)."/EJ_adverts/images/".$advert['EJ_advertImages']))
			{
				$image = "<img src=\"{$this->EJ_settings['instloc']}{$this->moduleloc}image.php/{$advert['EJ_advertImages']}?image={$this->EJ_settings['instloc']}{$this->moduleloc}images/{$advert['EJ_advertImages']}&amp;height=60&amp;width=80\" alt=\"{$advert['EJ_advertTitle']}\"/>";
			} else
			{
		...
		if ($count < 5)
		{
			for ($i = $count; $i<5; $i++)
			{
				$this->EJ_mysql->query("SELECT * FROM {$this->EJ_mysql->prefix}module_EJ_adverts WHERE EJ_advertHidden = 0 ORDER BY RAND()");
				$advert = $this->EJ_mysql->getRow();
				if (strrpos($advert['EJ_advertLoc'],"(")!=0)
				{
					$advert['locName'] = "Multiple Locations";
	...
	function show_new()
	{
		$this->EJ_mysql->query("SELECT * FROM {$this->EJ_mysql->prefix}module_EJ_adverts WHERE EJ_advertHidden = 0 ORDER BY EJ_advertId DESC LIMIT 5");
		while ($advert = $this->EJ_mysql->getRow())
		{
			if (strrpos($advert['EJ_advertLoc'],"(")!=0)
			{
	...
	{
		$this->EJ_mysql->query("SELECT * FROM {$this->EJ_mysql->prefix}module_EJ_adverts WHERE EJ_advertPoster = '{$_SESSION['userid']}'");
		switch ($this->EJ_mysql->numRows())
		{
			...
			case 1:
				...
				$content .= '
					<script src="'.$this->moduleloc.'EJ_adverts.js" language="javascript" type="text/javascript"></script>
					<link rel="stylesheet" href="modules/EJ_adverts/styles.php" type="text/css" />
					<div id="addadvert">
						<form name="add_form" id="add_form" action="?module=EJ_adverts&action=editadvert" method="post">
							<div id="addLeft">
								Click Image To Change<br/>
				...
				$this->EJ_mysql->query("SELECT hits FROM {$this->EJ_mysql->prefix}module_EJ_adverts_hits WHERE adId = '{$advert['EJ_advertId']}' and hitMonth = '".date("my", strtotime('-1 Month'))."'");
				if ($this->EJ_mysql->numRows() == 0)
				{
					$hits[1] = 0;
				}
			...
			default:
				$select = '
				<select onchange="document.location=\'?module=EJ_adverts&advertid=\'+this.value" style="margin-bottom: 5px;">';
				$i=0;
				while ($ad = $this->EJ_mysql->getRow())
				{
					if ($i==0)
					{
						if (($_REQUEST['module']=='EJ_adverts' and $_REQUEST['advertid']==$ad['EJ_advertId']) or !isset($_REQUEST['advertid']))
						{
							$advert=$ad;
							$i=1;
						}
	...
	function main_advert_categories()
	{
		...
		} else
		{
			...
			{
				$content .= '
					<li><a href="?module='.get_class($this).'&action=show_ads&cat='.$cat['catId'].'">'.$cat['catName'].'</a></li>';
			}
		}
		echo $content;
	}
